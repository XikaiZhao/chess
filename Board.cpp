...
void Board::makeMove(const Move& move) {
    ...
    if ( _board[move.curPos] == (isWhiteToMove ? 'P' : 'p')) {
        if ( newPosPiece == '-' && move.curPos%ncol != move.newPos%ncol) {
            // en passant
            _board[enPassantCapture] = '-';
            enPassantCapture = -1;
        }
        else if (std::abs(move.newPos/ncol - move.newPos/ncol) == 2) {
            enPassantCapture = isWhiteToMove ? move.newPos - ncol : move.newPos + ncol;
        }
    ...
    else if (_board[move.curPos] == (isWhiteToMove ? 'K' : 'k')) {
        enPassantCapture = -1;
        if (std::abs(move.newPos%ncol - move.curPos%ncol) == 2) {
            ...
            int ind = rook_c + move.newPos/ncol;
